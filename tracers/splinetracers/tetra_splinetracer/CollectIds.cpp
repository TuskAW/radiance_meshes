...
void CollectIds::trace_rays(const OptixTraversableHandle &handle,
                         ...
                         uint total_tris)
{
    CUDA_CHECK(cudaSetDevice(device));
    {
        ...
        params.tri_collection.size = total_tris;
        params.start_ids.data = start_ids;
        params.start_ids.size = num_rays;

        params.feature_size = 3;
        ...
        CUDA_CHECK( cudaMalloc( reinterpret_cast<void**>( &d_param ), sizeof( CIParams ) ) );
        ...
        OPTIX_CHECK( optixLaunch( pipeline, stream, d_param, sizeof( CIParams ), &sbt, num_rays, 1, 1 ) );
...
CollectIds::CollectIds(
        ...
        int8_t device,
        const Primitives &model) : device(device), context(context)
{
    // Initialize fields
    ...
    OptixModule module = nullptr;
    {
        ...
        const std::string input = CollectIds::load_ptx_data();
...
CollectIds::CollectIds(CollectIds &&other) noexcept
...
CollectIds::~CollectIds() noexcept(false) {
