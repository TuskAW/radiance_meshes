...
absl::string_view ExtractLine(absl::string_view &str) {
  ...
  // ASSIGN_OR_RETURN(line_sep_data, );
  // if (!NextLineEnd(str).ok())
  //   return NextLineEnd(str).status();
  // else
  //   auto line_sep_data = *NextLineEnd(str);
...
void SortComponentsToMortonOrder(GaussianScene &scene) {
  ...
  // TODO(dfutschik): Make it faster than array copy.
...
void NormalizeSceneData(GaussianScene &scene) {
  ...
  for (int cidx = 0; cidx < scene.num_elements; ++cidx) {
    ...
    // float sx = scene.scales[cidx][0];
    // float sy = scene.scales[cidx][1];
    // float sz = scene.scales[cidx][2];
    // float length1 = sqrtf(sx*sx + sy*sy);
    // float length2 = sqrtf(sx*sx + sz*sz);
    // float length3 = sqrtf(sy*sy + sz*sz);
    // float norm = fmax(length1, fmax(length2, length3));
    //
    // // Alphas.
    // float alpha = 0.95*sigmoid(scene.alphas[cidx]);
    // scene.alphas[cidx] = inv_opacity(alpha) / norm;
    // Scales.
    float norm = 9999;
    for (int s = 0; s < 3; ++s) {
      // scene.scales[cidx][s] = SCALE*std::min(std::exp(scene.scales[cidx][s]) + 2.5*1e-4, 100.0);
      scene.scales[cidx][s] = SCALE*std::min(
          softplus(scene.scales[cidx][s]) + 2.5*1e-5, 10.0);
      //scene.scales[cidx][s] = sigmoid(scene.scales[cidx][s]) * 10 + 1e-2;
      norm = std::min(scene.scales[cidx][s], norm);
    }
    // Alphas.
    float alpha = 0.5*sigmoid(scene.alphas[cidx]);
    scene.alphas[cidx] = inv_opacity(alpha) / norm;
...
GaussianScene ReadSceneFromFile(absl::string_view filename, bool approximate_morton_order) {
  ...
  // file::GetContents(filename, &file_contents, file::Defaults());
  ...
  // ASSIGN_OR_RETURN(line, ExtractLine(remaining_contents));
  ...
  while (true) {
    ...
    // std::cout << "Found property: " << line_parts[1] << " " << line_parts[2] << std::endl;;
  }
  GaussianScene scene;
  ...
  if (remaining_contents.size() < number_of_components * payload_size) {
    ...
    //     "File data is truncated, expected at least %d bytes, got %d.",
    //     number_of_components * payload_size, remaining_contents.size()));
  ...
  // if (approximate_morton_order) {
  //   std::cout << "Sorting components to Morton order.";
  //   SortComponentsToMortonOrder(scene);
  // }
  return scene;
}
