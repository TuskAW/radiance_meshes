...
class AlphaBlendTiledRender(torch.autograd.Function):
    ...
    def forward(ctx, 
        ...
        assert (render_grid.tile_height, render_grid.tile_width) in slang_modules.alpha_blend_shaders_linear, (
            ...
            f' {slang_modules.alpha_blend_shaders_linear.keys()}'
        )

        alpha_blend_tile_shader = slang_modules.alpha_blend_shaders_linear[(render_grid.tile_height, render_grid.tile_width)]
        st = time.time()
