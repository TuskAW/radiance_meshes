...
def contraction_jacobian_d_in_chunks(means, chunk_size=100000):
    jac_list = []
    ...
    for i in range(num_chunks):
        ...
        jac_chunk = jac_chunk.view([-1] + list(means.shape)[1:] + [means.shape[-1]])
        j_d_chunk = torch.det(jac_chunk).abs()
        jac_list.append(j_d_chunk)
